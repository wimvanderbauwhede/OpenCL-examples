...
{
            printf( "\nOptions:\n\n");
            printf( "   -t <n>             type of test:\n\n" );
            printf( "                      0 clEnqueue[Map,Unmap]\n" );
            printf( "                      1 clEnqueue[Read,Write]\n" );
            printf( "                      2 clEnqueueCopy\n" );
            printf( "                      3 clEnqueue[Read,Write], prepinned\n\n" );
            printf( "   -pcie or -dma      measure PCIe/interconnect B/W\n" );
            printf( "   -nwk <n>           number of CPU workers (max: 32)\n");
            printf( "   -nl <n>            number of timing loops\n");
            printf( "   -nr <n>            repeat each timing <n> times\n");
            printf( "   -nk <n>            number of loops in kernel\n");
            printf( "   -nb <n>            buffer size in bytes\n");
            printf( "   -nw <n>            # of wave fronts per SIMD\n");
            printf( "                      (default: 7)\n");
            printf( "   -l                 print complete timing log\n");
            printf( "   -s <n>             skip first <n> timings for average\n");
            printf( "                      (default: 1)\n");
            printf( "   -[if,of,cf] <n>    input, output, copy flags\n");
            printf( "                      (ok to use multiple):\n\n");

            for( int i=0; i < nFlags; i++ )
               printf( "                      %d %s\n", i, flags[i].s );
            printf( "\n" );

            printf( "   -m                 always map as MAP_READ | MAP_WRITE\n");
            printf( "   -db                disable host mem B/W baseline\n");
            printf( "   -d                 Number of GPU devices \n");
                  ...
                  bool quiet )
{
    ...
    *ptr = (void * ) clEnqueueMapBuffer( queue,
    ...
    if( !quiet )
        tlog->Timer( "%32s  %lf s [ %8.2lf GB/s ]\n", "clEnqueueMapBuffer:", t1.GetElapsedTime(), nBytes, 1 );
}
...
void printHeader()
{
   ...
   printf( "GPU work items:      %d\n", nThreads );
   printf( "Buffer size:         %d\n", nBytes );
   printf( "CPU workers:         %d\n", nWorkers );
   printf( "Timing loops:        %d\n", nLoops );
   printf( "Repeats:             %d\n", nRepeats );

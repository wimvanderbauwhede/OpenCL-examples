...
MatrixMultiplication::setupCL(void)
{
    ...
    if(!sampleCommon->checkVal(
            status,
            CL_SUCCESS,
            "clEnqueueWriteBuffer failed. (inputBuffer0)"))
        return SDK_FAILURE;
    ...
    if(!sampleCommon->checkVal(
            status,
            CL_SUCCESS,
            "clEnqueueWriteBuffer failed. (inputBuffer1)"))
        return SDK_FAILURE;
...
main(int argc, char * argv[])
{
    ...
    if(clMatrixMultiplication.isDumpBinaryEnabled())
    {
        return clMatrixMultiplication.genBinaryImage();
    }
    else
    {
        if(clMatrixMultiplication.setup()==SDK_FAILURE)
          return SDK_FAILURE;
        int state = clMatrixMultiplication.run();
        if(state == SDK_FAILURE)
          return SDK_FAILURE;
        else
        {
            if(clMatrixMultiplication.verifyResults()==SDK_FAILURE)
                return SDK_FAILURE;
        }
        if(clMatrixMultiplication.cleanup()==SDK_FAILURE)
          return SDK_FAILURE;

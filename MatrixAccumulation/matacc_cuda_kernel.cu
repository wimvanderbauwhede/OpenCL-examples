...
__global__ void mataccKernel21 (
    float *mA, // although this is constant, setting it to __constant results in CL_OUT_OF_RESOURCES
    float *mC,
    const unsigned int mWidth) {
	unsigned int g_id = blockIdx.x;
    float elt=0.0;
...
*/
// Linear
__global__ void mataccKernel14 (
    float *mA, // although this is constant, setting it to __constant results in CL_OUT_OF_RESOURCES
    float *mC,
    const unsigned int mWidth) {
	unsigned int g_id = blockIdx.x;
	unsigned int nunits=gridDim.x;
//    printf("mWidth:%d,g_id:%d,nunits:%d\n",mWidth,g_id,nunits);
    float elt=0.0;
...
// linear, float4
__global__ void mataccKernel4 (
    __device__ float4 *mA,
    __device__ float *mC,
    const unsigned int mWidth) {
	unsigned int g_id = get_group_id(0);
	unsigned int nunits=get_num_groups(0);
...
__global__ void mataccKernel5 (
    __device__ float8 *mA,
    __device__ float *mC,
    const unsigned int mWidth) {
	unsigned int g_id = get_group_id(0);
	unsigned int nunits=get_num_groups(0);
...
__global__ void mataccKernel6 (
    __device__ float16 *mA,
    __device__ float *mC,
    const unsigned int mWidth) {
	unsigned int g_id = get_group_id(0);
	unsigned int nunits=get_num_groups(0);
...
__global__ void mataccKernel11 (
	__device__ float16 *mA,
	__device__ float *mC,
	const unsigned int mWidth) {
	unsigned int g_id = get_group_id(0);
...
__global__ void mataccKernel10 (
    __device__ float16 *mA,
    __device__ float *mC,
    const unsigned int mWidth) {
	const int nthreads=16;
...
// float16, 64 threads
__global__ void mataccKernel13 (
    __device__ float16 *mA,
    __device__ float *mC,
    const unsigned int mWidth) {
...
__global__ void mataccKernel12 (
    __device__ float16 *mA,
    __device__ float *mC,
    const unsigned int mWidth) {
	const int nthreads=16;
...
// The kernels below use threads
// linear, 16 threads
__global__ void mataccKernel15 (
    __device__ float *mA,
    __device__ float *mC,
    const unsigned int mWidth) {
	const int nthreads=16;
...
// linear, 32 threads
__global__ void mataccKernel16 (
    __device__ float *mA,
    __device__ float *mC,
    const unsigned int mWidth) {
	const int nthreads=32;
...
// linear, 64 threads
__global__ void mataccKernel17 (
    __device__ float *mA,
    __device__ float *mC,
    const unsigned int mWidth) {
	const int nthreads=64;
...
// linear, 128 threads
__global__ void mataccKernel18 (
    __device__ float *mA,
    __device__ float *mC,
    const unsigned int mWidth) {
...
// linear, 256 threads
__global__ void mataccKernel19 (
    __device__ float *mA,
    __device__ float *mC,
    const unsigned int mWidth) {
...
// float16, 128 threads
__global__ void mataccKernel20 (
	    __device__ float16 *mA,
	    __device__ float *mC,
	    const unsigned int mWidth) {
		const int nthreads=128;

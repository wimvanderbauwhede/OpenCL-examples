...
# Create the buffers 
my $mA_buf = $ocl->makeReadBuffer(float, $mSize,$mA); # read from by the kernel
my $mC=[];
$#{$mC}=$nunits;
my $mC_buf = $ocl->makeWriteBuffer(float, $nunits,$mC); # written to by the kernel
#say @{$mC};die;
...
for my $run (1 .. $nruns) {
# Run the kernel
    $ocl->runKernel($mA_buf,$mC_buf,$mW_const);
#--------------------------------------------------------------------------------;

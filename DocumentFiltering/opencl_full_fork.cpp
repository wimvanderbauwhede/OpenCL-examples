...
#else
    word_t *tempProfile = new word_t[PROF_SZ];
    for (unsigned int i=0;i<PROF_SZ;i++) {
    	tempProfile[i]=profile->at(i);
    }
#endif
...
#else
        int profileSize = sizeof(word_t) * PROF_SZ;
#endif
...
#ifdef WATERLOO
#ifdef BLOOM_FILTER
        program.build(devices, "-DBLOOM_FILTER -DWATERLOO");
#else
        program.build(devices, "-DWATERLOO");
#endif
#else
#ifdef BLOOM_FILTER
